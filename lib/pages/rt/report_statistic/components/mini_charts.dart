...
class MiniDonutCard extends StatelessWidget {
  ...
  const MiniDonutCard({required this.title, required this.data, super.key});

  @override
  Widget build(BuildContext context) {
    final segments = data.entries
        .map((e) => Segment(e.key, e.value, _pickColor(e.key)))
        .toList();
    return Container(
...
class MiniBarCard extends StatelessWidget {
  final String title;
  final Map<String, double> data;
  const MiniBarCard({required this.title, required this.data, super.key});
        ...
        children: [
          ...
          Column(
            children: data.entries.map((e) {
              final widthFactor = (max <= 0)
                  ? 0.0
                  : (e.value / max).clamp(0.0, 1.0);
              return Padding(
