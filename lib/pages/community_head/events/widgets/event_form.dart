...
class EventForm extends StatefulWidget {
  ...
  const EventForm({this.initialEvent, required this.onSubmit, this.submitLabel = 'Simpan', super.key});
...
class _EventFormState extends State<EventForm> {
  ...
  Widget build(BuildContext context) {
    final InputDecoration Function(String) _dec = (label) => buildInputDecoration(label);
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 8),
                TextFormField(controller: _titleCtrl, decoration: _dec('Judul Kegiatan'), validator: (v) => (v == null || v.trim().isEmpty) ? 'Judul diperlukan' : null),
                const SizedBox(height: 12),

                Row(children: [
                  Expanded(child: GestureDetector(onTap: _pickDate, child: AbsorbPointer(child: TextFormField(decoration: _dec('Tanggal'), controller: TextEditingController(text: DateFormatter.formatFull(_date)))))),
                  const SizedBox(width: 12),
                  Expanded(child: GestureDetector(onTap: _pickTime, child: AbsorbPointer(child: TextFormField(decoration: _dec('Waktu'), controller: TextEditingController(text: _time.format(context)))))),
                ]),

                const SizedBox(height: 12),
                TextFormField(controller: _locationCtrl, decoration: _dec('Lokasi'), validator: (v) => (v == null || v.trim().isEmpty) ? 'Lokasi diperlukan' : null),
                const SizedBox(height: 12),
                TextFormField(controller: _descriptionCtrl, decoration: _dec('Deskripsi'), maxLines: 6, validator: (v) => (v == null || v.trim().isEmpty) ? 'Deskripsi diperlukan' : null),
              ],
            ),
          ),
